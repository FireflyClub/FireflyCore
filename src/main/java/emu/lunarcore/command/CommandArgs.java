    ...
    public CommandArgs(Player sender, List<String> args, @Nullable Context ctx) {
        ...
        if (targetUid != 0) {
            if (LunarCore.getGameServer() != null) {
                onlineTarget = LunarCore.getGameServer().getOnlinePlayerByUid(targetUid);
                target = LunarCore.getGameServer().getPlayerByUid(targetUid, true);
            }
        } else {
            onlineTarget = sender;
            target = sender;

            // No target then check if console
            if (sender != null) {
                targetUid = sender.getUid();
            }
        }
