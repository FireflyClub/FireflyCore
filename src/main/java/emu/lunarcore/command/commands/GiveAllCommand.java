    ...
    public void execute(CommandArgs args) {
        ...
        switch (type) {
            ...
            case "lc", "lightcones" -> {
                ...
                var tab = args.getOnlineTarget().getInventory().getTabByItemType(ItemMainType.Equipment);
            ...
            case "r", "relics" -> {
                // Make sure we dont go over the inventory limit
                var tab = args.getOnlineTarget().getInventory().getTabByItemType(ItemMainType.Relic);
                if (tab.getSize() >= tab.getMaxCapacity()) {
                    args.sendMessage(target.getName() + " has too many of this item type");
                    return;
                }
            ...
            case "a", "characters", "avatars" -> {
                ...
                for (var excel : GameData.getAvatarExcelMap().values()) {
                    ...
                    if (excel.getAvatarID() > 2000 && (excel.getAvatarID() < 8001 || excel.getAvatarID() > 8006)) continue; // Skip probation avatars
