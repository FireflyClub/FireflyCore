...
public class ChatManager extends BasePlayerManager {
    ...
    public void sendChat(int targetUid, String text) {
        ...
        // Check if command to server console
        if (targetUid == GameConstants.SERVER_CONSOLE_UID) {
            // Display message for sender
            ChatMessage message = new ChatMessage(this.getPlayer().getUid(), GameConstants.SERVER_CONSOLE_UID, text);
        ...
        // Check if command to server chat
        if (targetUid == GameConstants.SERVER_CHATROOM_UID) {
            ChatRoomManager.handleChatRoomMsg(getPlayer(), targetUid, text);
            return;
        }
    ...
    public void sendChat(int targetUid, int emote) {
        ...
        if (targetUid == GameConstants.SERVER_CONSOLE_UID) {
            // Display message for sender
            ChatMessage message = new ChatMessage(this.getPlayer().getUid(), GameConstants.SERVER_CONSOLE_UID, emote);
            this.addChatMessage(message);
        ...
        // Check if command to server chat
        if (targetUid == GameConstants.SERVER_CHATROOM_UID) {
            ChatRoomManager.handleChatRoomEmote(getPlayer(), targetUid, emote);
            return;
        }
