...
public class Player implements Tickable, Syncable {
    ...
    // This method won't refresh account date to radis at once
    public static Account getAccountByDatabase(int playerUid) {
        Player player = LunarCore.getGameDatabase().getObjectByField(Player.class, "_id", playerUid);
        String accountUid = player.getAccountUid();
        Account account = LunarCore.getAccountDatabase().getObjectByField(Account.class, "_id", accountUid);
        return account;
    }
    ...
    private void initUid() {
        ...
        if (nextUid == GameConstants.SERVER_CONSOLE_UID || nextUid == GameConstants.SERVER_CHATROOM_UID) {
            nextUid = 0;
        }
