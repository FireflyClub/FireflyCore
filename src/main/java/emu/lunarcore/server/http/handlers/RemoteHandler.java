...
public final class RemoteHandler implements Handler {
    ...
    public void handle(@NotNull Context ctx) throws Exception {
        ...
        if (key == null || key.isEmpty()) {
            ctx.json(new RemoteResponse(404, "The password was not entered"));
            return;
        }
        if (command == null || command.isEmpty()) {
            ctx.json(new RemoteResponse(404, "The command was not entered"));
            return;
        }
            ...
            if (sender != null) {
                ...
                if (pwd != null) {
                    if (pwd.equals(encrypted_key)){
                        ...
                        ctx.json(new RemoteResponse(201, "The key is incorrect"));
                        return;
                    }

                } else {
                    ctx.json(new RemoteResponse(201, "The key is not set"));
                    return;
