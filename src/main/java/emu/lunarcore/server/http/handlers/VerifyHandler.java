...
public final class VerifyHandler implements Handler {
    ...
    public void handle(@NotNull Context ctx) throws Exception {
        ...
        if (tmp_code == null || tmp_code.isEmpty()) {
            ctx.json(new RemoteResponse(404, "The temp code was not entered"));
            return;
        }
        if (set_password == null || set_password.isEmpty()) {
            ctx.json(new RemoteResponse(404, "The player remote password was not entered"));
            return;
        }
        ...
        try {
            ...
            if (sender != null) {
                String code = ApplyHandler.getCodeByUid(tmp_uid);
                ...
                if (code != null) {
                    if (code.equals(tmp_code)) {
                        ...
                        ctx.json(new RemoteResponse(201, "The code is incorrect"));
                        return;

                    }

                } else {
                    ctx.json(new RemoteResponse(201, "The code is not set"));
                    return;
